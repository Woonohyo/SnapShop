...
/**
 * {@code PullToRefreshConfigXmlParser} does parsing pulltorefresh.xml as
 * {@code XmlPullNode}s
 * 
 * @author Wonjun Kim
 */
final class PullToRefreshConfigXmlParser extends XmlPullNodeParser<PullToRefreshNode> {
	...
	protected PullToRefreshNode getResult() {
		return new PullToRefreshNode(result.loadingLayoutClazzNameMap, result.indicatorLayoutClazzNameMap,
				result.googleStyleViewLayoutClazzNameMap, result.googleStyleProgressLayoutClazzNameMap);
	}
	...
	/**
	 * Callback of the node 'PullToRefresh/IndicatorLayouts/layout' and
	 * 'PullToRefresh/LoadingLayouts/layout'
	 * 
	 * @author Wonjun Kim
	 *
	 */
	private static class LayoutNodeCallback implements XmlPullNode.XmlPullNodeCallback {
		 ...
		 *            Must not be null and be clean for which put new values
		 */
		...
		public void process(XmlPullParser parser) throws XmlPullParserException, IOException {
			...
			for (int i = 0; i < attributesCount; ++i) {
				...
				// The 'name' attribute is as a layout code of each layout's
				// class
